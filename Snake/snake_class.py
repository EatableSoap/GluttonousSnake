...
class Snake:
    ...
    # 绘制背景
    def put_a_background(self, can, color='Dimgray'):
    ...
    def move_snake(self, snke_list, direc, rush):
        if direc != [0, 0]:
            ...
            if not self.snake_eat(snke_list, self.Food_pos):
                ...
                self.draw_a_unit(self.canvas, snke_list[-1][0], snke_list[-1][1], unit_color="Dimgray")
                snke_list.pop(-1)
    ...
    def __init__(self, row=40, column=40,Fps=100,Unit_size=20):
        ...
        self.energy_label = tk.Label(self.win, textvariable=self.str_energy, font=('楷体', Unit_size), width=10, height=1)
